...
discount <- function(df, rate = 0.03, cost = 1){
...
monte.carlo.icer.ma <- function(num, rate = 0.04, outcome, timeframe = 20){ # time frame = 2019-2039
  ...
  for(i in 1:num){
    ...
    compared.cost <- compare.costs(list.by.scenario)
    temp.atlas <- icer(compared.cost[[1]], outcome[[1]][,1:(timeframe+1)], timeframe) %>% as.data.frame()
    temp.su <- icer(compared.cost[[2]], outcome[[2]][ ,1:(timeframe+1)],timeframe) %>% as.data.frame()
    if(i == 1){
      icer.atlas <- temp.atlas
      icer.su <- temp.su
    }else{
